...
static const ng_uint64_t DOUBLE_POW5_INV_SPLIT2[15][2] = {
...
static const ng_uint32_t POW5_INV_OFFSETS[19] = {
...
static const ng_uint64_t DOUBLE_POW5_SPLIT2[13][2] = {
...
static const ng_uint32_t POW5_OFFSETS[21] = {
...
static const ng_uint64_t DOUBLE_POW5_TABLE[POW5_TABLE_SIZE] = {
...
static inline void double_computePow5(const ng_uint32_t i, ng_uint64_t* const result) {
  const ng_uint32_t base = i / POW5_TABLE_SIZE;
  const ng_uint32_t base2 = base * POW5_TABLE_SIZE;
  const ng_uint32_t offset = i - base2;
  const ng_uint64_t* const mul = DOUBLE_POW5_SPLIT2[base];
  if (offset == 0) {
    result[0] = mul[0];
    result[1] = mul[1];
    return;
  }
  const ng_uint64_t m = DOUBLE_POW5_TABLE[offset];
  ...
  result[0] = (ng_uint64_t) shiftedSum;
  result[1] = (ng_uint64_t) (shiftedSum >> 64);
}

// Computes 5^-i in the form required by Ryu, and stores it in the given pointer.
static inline void double_computeInvPow5(const ng_uint32_t i, ng_uint64_t* const result) {
  const ng_uint32_t base = (i + POW5_TABLE_SIZE - 1) / POW5_TABLE_SIZE;
  const ng_uint32_t base2 = base * POW5_TABLE_SIZE;
  const ng_uint32_t offset = base2 - i;
  const ng_uint64_t* const mul = DOUBLE_POW5_INV_SPLIT2[base]; // 1/5^base2
  if (offset == 0) {
    result[0] = mul[0];
    result[1] = mul[1];
    return;
  }
  const ng_uint64_t m = DOUBLE_POW5_TABLE[offset]; // 5^offset
...
static inline void double_computePow5(const ng_uint32_t i, ng_uint64_t* const result) {
  ...
  const ng_uint64_t m = DOUBLE_POW5_TABLE[offset];
  ng_uint64_t high1;
  const ng_uint64_t low1 = umul128(m, mul[1], &high1);
  ng_uint64_t high0;
  const ng_uint64_t low0 = umul128(m, mul[0], &high0);
...
static inline void double_computeInvPow5(const ng_uint32_t i, ng_uint64_t* const result) {
  ...
  const ng_uint64_t low1 = umul128(m, mul[1], &high1);
  ng_uint64_t high0;
  const ng_uint64_t low0 = umul128(m, mul[0] - 1, &high0);
  const ng_uint64_t sum = high0 + low1;
  if (sum < high0) {
    ++high1; // overflow into high1
  }
